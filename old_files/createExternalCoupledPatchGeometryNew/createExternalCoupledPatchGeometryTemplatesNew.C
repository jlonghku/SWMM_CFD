...
#include "createExternalCoupledPatchGeometryTemplatesNew.H"
...
)
{
    ...
    if (fieldObjbjects.lookup(fieldName) != nullptr)
    {
        ...
        forAll(bf, patchi)
        {
            if (isA<externalCoupledMixedNewFvPatchField<Type>>(bf[patchi]))
            {

                Info<< "Generating external coupled geometry for field "
                    << fieldName << endl;

                const externalCoupledMixedNewFvPatchField<Type>& pf =
                    refCast<const externalCoupledMixedNewFvPatchField<Type>>
                    (
                        bf[patchi]
                    );
        ...
        if (processed != 1)
        {
            ...
            Info<< "Field " << fieldName << " found, but does not have any "
                << externalCoupledMixedNewFvPatchField<Type>::typeName
