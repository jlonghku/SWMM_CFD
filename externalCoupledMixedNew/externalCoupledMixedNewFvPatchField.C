...
#include "externalCoupledMixedNewFvPatchField.H"
...
template <class Type>
Foam::word Foam::externalCoupledMixedNewFvPatchField<Type>::lockName() const
{
    return "OpenFOAM_" + this->db().time().caseName();
}

template<class Type>
Foam::string
Foam::externalCoupledMixedNewFvPatchField<Type>::patchKey = "# Patch: ";


// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

template<class Type>
Foam::fileName Foam::externalCoupledMixedNewFvPatchField<Type>::baseDir() const
{
...
template<class Type>
void Foam::externalCoupledMixedNewFvPatchField<Type>::setMaster
(
...
template<class Type>
void Foam::externalCoupledMixedNewFvPatchField<Type>::writeGeometry
(
    OFstream& osPoints,
    OFstream& osFaces
) const
...
template<class Type>
Foam::fileName Foam::externalCoupledMixedNewFvPatchField<Type>::lockFile() const
{
...
template<class Type>
void Foam::externalCoupledMixedNewFvPatchField<Type>::createLockFile() const
{
...
template<class Type>
void Foam::externalCoupledMixedNewFvPatchField<Type>::removeLockFile() const
{
    if (!master_ || !Pstream::master())
    {
        return;
...
template<class Type>
void Foam::externalCoupledMixedNewFvPatchField<Type>::startWait() const
{
...
template<class Type>
void Foam::externalCoupledMixedNewFvPatchField<Type>::wait() const
{
    const fileName fName(lockFile());
    label found = 0;
    label totalTime = 0;
...
template<class Type>
void Foam::externalCoupledMixedNewFvPatchField<Type>::initialiseRead
(
    IFstream& is
) const
{
...
template<class Type>
void Foam::externalCoupledMixedNewFvPatchField<Type>::readData
(
...
template<class Type>
void Foam::externalCoupledMixedNewFvPatchField<Type>::writeData
(
    const fileName& transferFile
) const
{
...
template<class Type>
void Foam::externalCoupledMixedNewFvPatchField<Type>::writeHeader
(
    OFstream& os
) const
{
    os  << "# Values: magSf value snGrad nf" << endl;
}
...
template<class Type>
Foam::externalCoupledMixedNewFvPatchField<Type>::
externalCoupledMixedNewFvPatchField
(
    const fvPatch& p,
...
externalCoupledMixedNewFvPatchField
(
    const externalCoupledMixedNewFvPatchField& ptf,
    const fvPatch& p,
    const DimensionedField<Type, volMesh>& iF,
...
:
    ...
    calcInterval_(ptf.calcInterval_),
...
:
    ...
    calcInterval_(dict.lookupOrDefault("calcInterval", 0.0)),
...
externalCoupledMixedNewFvPatchField
(
    const externalCoupledMixedNewFvPatchField& ecmpf
)
:
    ...
    calcInterval_(ecmpf.calcInterval_),
...
template<class Type>
void Foam::externalCoupledMixedNewFvPatchField<Type>::initialise
(
    const fileName& transferFile
)
{
...
template<class Type>
void Foam::externalCoupledMixedNewFvPatchField<Type>::evaluate
(
    const Pstream::commsTypes comms
)
{
    const scalar findex = (this->db().time().value() - this->db().time().startTime().value()) / calcInterval_;    
    if (!initialised_ || (this->db().time().timeIndex() % calcFrequency_ == 0 && calcInterval_ == 0.0) || (mag(findex-label(findex+0.5))<=1e-8&& calcInterval_ > 0))
    { 
...
template<class Type>
void Foam::externalCoupledMixedNewFvPatchField<Type>::transferData
(
    OFstream& os
) const
{
    ...
    os << "[\"" << this->internalField().name() << "\"," << nl;
    os<<"\""<<this->patch().name()<<"\","<<nl;
    os<<"{\"model\":\"OpenFOAM\","<<nl;
    if (Pstream::parRun())
    {
        int tag = Pstream::msgType() + 1;
        ...
        List<Field<vector>> Sfs(Pstream::nProcs());
        Sfs[Pstream::myProcNo()].setSize(this->patch().size());
        Sfs[Pstream::myProcNo()] = this->patch().Sf();
        Pstream::gatherList(Sfs, tag);
        ...
        if (Pstream::master())
        {
            forAll(values, proci)
            {
                ...
                const Field<vector> &Sf = Sfs[proci];
                os <<  "\"magSf_OpenFOAM\":[" << nl;
                forAll(value, i)
                {
                    os << magSf[i] << "," << nl;
                }
                os << "]," << nl << "\"value_OpenFOAM\":[" << nl;
                forAll(value, i)
                {
                    os << value[i] << "," << nl;
                }
                os << "]," << nl << "\"snGrad_OpenFOAM\":[" << nl;
                forAll(value, i)
                {
                    os << snGrad[i] << "," << nl;
                }
                os << "]," << nl << "\"Sf_OpenFOAM\":[" << nl;
                forAll(value, i)
                {
                    os << Sf[i] << "," << nl;
                }
    ...
    else
    {
        ...
        const Field<vector> &Sf(this->patch().Sf());
        os << "\"magSf_OpenFOAM\":[" << nl;
        forAll(value, i)
        {
            os << magSf[i] << "," << nl;
        ...
        // os << "magSf: "<<magSf << nl<< "value: "<< value << nl<< "Sf: "<< snGrad << nl<<nl;
        os.flush();
    }
}
...
template<class Type>
void Foam::externalCoupledMixedNewFvPatchField<Type>::writeGeometry() const
{
    const volFieldType& cvf =
        static_cast<const volFieldType&>(this->internalField());
    ...
    OFstream osPoints(baseDir()/"patchPoints_OpenFOAM_"+this->db().time().caseName()+"_"+this->internalField().name());
    OFstream osFaces(baseDir()/"patchFaces_OpenFOAM_"+this->db().time().caseName()+"_"+this->internalField().name());
...
template<class Type>
void Foam::externalCoupledMixedNewFvPatchField<Type>::write(Ostream& os) const
{
    ...
    writeEntry(os, "calcInterval", calcInterval_);
