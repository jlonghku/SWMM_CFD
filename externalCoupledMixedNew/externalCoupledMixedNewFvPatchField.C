    ...
    if (Pstream::parRun())
    {
        ...
        if (Pstream::master())
        {
            forAll(values, proci)
            {
                ...
                os << "magSf: "<<magSf << nl<< "value: "<< value << nl<< "snGrad: "<< snGrad << nl<<nl;
            }
...
void Foam::externalCoupledMixedNewFvPatchField<Type>::writeGeometry() const
{
    ...
    OFstream osPoints(baseDir()/"patchPoints_OpenFOAM_"+this->db().time().caseName());
    OFstream osFaces(baseDir()/"patchFaces_OpenFOAM_"+this->db().time().caseName());
